...
def load_crustal_parameters(filepath,node_spacing):
    ...
    profile_length = int((profile_extent - (profile_extent % node_spacing))+node_spacing)
    ...
    for i,distance in enumerate(profile):
        ...
        elastic_thicknesses[i] = get_y_val(data['distance'],data['elastic_thickness'],distance)
...
def draw_graph(profile, deflections, loads, elastic_thicknesses, filename):
    check = bool(np.all(np.round(elastic_thicknesses,0) == np.round(elastic_thicknesses[0],0)))
...
def main():
    parser = argparse.ArgumentParser(prog='pyflex2D',
    ...
    parser.add_argument('input_file',help='Path to input file containing topographic profile and flexural rigidity information. All variables MUST be in m.')
    parser.add_argument('--output-destination','-o',help='Destination to write output files to. Defaults to execution directory.',default='.')
    parser.add_argument('--node-spacing',help='How closely spaced the grid nodes are. Defualts to 5000 m',type=float,default=5000)
    parser.add_argument('--fill-density',help='Density of the basin fill. Defaults to 2700 kg m^-3',type=float,default=2700)
    parser.add_argument('--mantle-density',help='Density of the mantle. Defaults to 3300 kg m^-3',type=float,default=3300)
    parser.add_argument('--youngs-modulus',help='Youngs Modulus. Defaults to 70e9 Pa',type=float,default=70.0e9)
    parser.add_argument('--gravity',help='Gravitational acceleration. Defaults to 9.81 m s^-2',type=float,default=9.81)
    parser.add_argument('--poisson',help='Poissons Ratio. Defaults to 0.25',type=float,default=0.25)
    ...
    # Model parameters
    print('Reading input...')
    x, load_height, load_density, elastic_thicknesses = load_crustal_parameters(args.input_file, args.node_spacing)
    ...
    draw_graph(x, deflections, load_height, elastic_thicknesses, f'{args.output_destination}/flexure_graph.svg')
    deflection_data = pd.DataFrame({'distance_m':x,
